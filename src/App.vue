<template>
  ...
  <div v-if="!user" class="login-wrap">
    <div class="login-card">
      ...
      <div class="field">
        ...
        <input v-model="serverUrl" placeholder="http://your-vps-ip:4000/api" @blur="saveServerUrl" />
      </div>
      <div class="field">
        <label>Email</label>
        <input v-model="loginForm.email" type="email" placeholder="admin@example.com" @keyup.enter="doLogin" />
      </div>
      <div class="field">
        <label>Password</label>
        <input v-model="loginForm.password" type="password" placeholder="password" @keyup.enter="doLogin" />
      </div>
  ...
  <div v-else class="app">
    ...
    <div v-if="stats" class="stats-bar">
      ...
      <div v-for="(count, action) in topActions" :key="action" class="stat-card action">
    ...
    <div v-else class="table-wrap">
      <table class="log-table">
        ...
        <tbody>
          ...
          <tr v-for="log in logs" :key="log._id" :class="{ 'row-fail': log.status === 'FAILURE' }">
    ...
    <div v-if="pagination && pagination.totalPages > 0" class="pagination">
      <button class="btn-ghost" :disabled="!pagination.hasPrevPage" @click="goToPage(currentPage - 1)">Prev</button>
      ...
      <button class="btn-ghost" :disabled="!pagination.hasNextPage" @click="goToPage(currentPage + 1)">Next</button>
...
import { login, fetchLogs, fetchStats, getToken, getStoredUser, getBaseUrl, setBaseUrl, clearAuth } from './api'
import type { LogEntry, Pagination, Filters } from './types'

const ACTIONS = [
  'LOGIN','LOGOUT','START_TEST','SUBMIT_TEST','SAVE_ANSWER',
  'UPLOAD_AUDIO','UPDATE_PROFILE','DELETE_ACCOUNT','ACCESS_RESOURCE','ADMIN_ACTION','ERROR',
]
...
function saveServerUrl() { setBaseUrl(serverUrl.value) }

async function doLogin() {
  if (!serverUrl.value) { loginError.value = 'Enter server URL first'; return }
...
const stats = ref<{ statusStats: Record<string, number>; actionStats: Record<string, number> } | null>(null)
...
async function loadAll() { await Promise.all([loadLogs(), loadStats()]) }
...
const serverLabel = computed(() => {
  try { return new URL(serverUrl.value).host } catch { return serverUrl.value }
})

const topActions = computed(() => {
  if (!stats.value) return {}
  return Object.fromEntries(
    Object.entries(stats.value.actionStats).sort((a, b) => b[1] - a[1]).slice(0, 3)
  )
})
...
function actionClass(action: string): string {
  const map: Record<string, string> = {
    ...
    ACCESS_RESOURCE: 'badge-access', ADMIN_ACTION: 'badge-admin', ERROR: 'badge-fail',
  }
...
.logo-icon { font-size: 22px; font-weight: 800; color: #6366f1; display: block; margin-bottom: 6px; }
...
.user-badge { font-size: 13px; color: #a0aec0; display: flex; align-items: center; gap: 6px; }
.user-badge em { font-style: normal; color: #68d391; font-size: 11px; }
.role-dot { width: 8px; height: 8px; border-radius: 50%; background: #a0aec0; }
...
.stat-card {
  ...
  background: #f8fafc; border: 1px solid #e5e7eb; min-width: 90px;
}
...
/* Filters */
.filters-panel { background: #fff; border-bottom: 1px solid #e5e7eb; padding: 16px 24px; }
.filters-grid { display: flex; flex-wrap: wrap; gap: 12px; margin-bottom: 12px; }
.filters-grid .field { min-width: 150px; flex: 1 1 150px; }
.filter-actions { display: flex; gap: 8px; }

/* Fields */
.field { display: flex; flex-direction: column; gap: 4px; }
.field label { font-size: 11px; font-weight: 600; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px; }
...
.btn-primary {
  ...
  padding: 8px 18px; font-size: 13px; font-weight: 600; cursor: pointer; transition: background 0.15s;
}
...
.btn-ghost {
  ...
  padding: 8px 14px; font-size: 13px; cursor: pointer; color: #374151; transition: background 0.15s;
}
...
.pagination {
  ...
  gap: 6px; padding: 16px 24px; background: #fff;
  border-top: 1px solid #e5e7eb; flex-wrap: wrap;
}
.page-numbers { display: flex; gap: 4px; }
.page-btn {
  min-width: 34px; height: 34px; border: 1px solid #d1d5db;
...
@media (max-width: 640px) {
  .top-bar, .filters-panel, .table-wrap { padding: 12px; }
